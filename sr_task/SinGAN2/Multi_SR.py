...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_name', help = 'training image name', default = " ")
    ...
    image_folder = glob.glob(opt.input_dir+'/*.jpg')
    #for i in range(len(image_folder)):
        #print(image_folder[i]+'\n')
    
    for i in range(len(image_folder)):
        opt.input_name = image_folder[i][13:]
        print(opt)
        Gs = []
        Zs = []
        reals = []
        ...
        if dir2save is None:
            ...
            dir2save = functions.generate_dir2save(opt)
            plt.imsave('%s/%s_HR.png' % (dir2save,opt.input_name[:-4]), functions.conver_image_np(out.detach()), vmin=0, vmax=1)
